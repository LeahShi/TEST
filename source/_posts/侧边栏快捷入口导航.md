...
最先想到的是使用，用a标签添加锚点跳转对应点的方法，这样虽然实现了串口滚动精确定位，但选中缺没有高亮。
...
$(document).ready(function(){
    ...
    for(var i=1;i<length;i++){
        if(i==1){
            ...
            //最后一项
            if(i==length-1){
                if(Height>$(".module"+i).height()+$(".footer").height()){
                    arr[i]=arr[i]-(Height- $(".module"+i).height()- $(".footer").height());
                    //console.log(arr[i]);
                }else{
                    arr[i] =arr[i-1]+$(".module"+i).height()+60;
                }
                //arr[i]=arr[i-1]+$(".module"+i).height()+60-$(".footer").height();
            }
        }
